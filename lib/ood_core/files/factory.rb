    ...
    class Factory
      ...
      class << self
        ...
        def build(config)
          ...
          adapter = c.fetch(:adapter) { raise AdapterNotSpecified, 'files configuration does not specify adapter' }.to_s

          path_to_adapter = "ood_core/files/adapters/#{adapter}"
          begin
            ...
            raise LoadError,
                  "Could not load '#{adapter}'. Make sure that the job adapter in the configuration file is valid."
          end
